(ns witan.send.model-test
  (:require [witan.send.model :as m]
  ...
  (let [library (m/model-library)
    ...
    (testing "The catalog entries are existing functions"
      (let [library-fns (map #(:witan/impl %) funs)
            model-ns-list (map str (keys (ns-publics 'witan.send.send)))
            model-ns-fns (map #(keyword (format "witan.send.send/%s" %)) model-ns-list)]
