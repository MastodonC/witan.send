...
(defn foo [year transitions-data]
  (->> transitions-data
       (filter #(is-send-and-is-year? % year))
       (map :academic-year-1)
       count))
...
(defn population-line-plot
  ...
  (let [historic-years (vec (distinct (map :calendar-year historic-data)))
        ...
        max-max (apply max (map :max future-projections-data))]
    (gg4clj/render [[:require "ggforce"]
                    [:<- :df
                     ...
                     (gg4clj/r+ [:ggplot {:data :df} [:aes {:x :year :y :mean}]]
                                ...
                                [:scale_y_continuous {:name "Total SEND population" :limits [:c 0 max-max]}]
...
(defn send-cost-plot
  ...
  (let [proj-years (vec (#(range % (+ (count cost-data) %)) (inc (last years))))
        low-ci (vec  (map #(/ % 1000000) (map :low-ci cost-data)))
        q1 (vec (map #(/ % 1000000) (map :q1 cost-data)))
        median  (vec (map #(/ % 1000000) (map :median cost-data)))
        q3 (vec (map #(/ % 1000000) (map :q3 cost-data)))
        high-ci (vec (map #(/ % 1000000) (map :high-ci cost-data)))
        gg2clj-data-frame {:year proj-years :ymin low-ci :lower q1 :middle median :upper q3 :ymax high-ci}
        max-max (last high-ci)
    ...
    (gg4clj/render [[:<- :df
                    ...
                    (gg4clj/r+ [:ggplot {:data :df} [:aes {:x :year}]]
                               ...
                               [:scale_y_continuous {:name "Total Projected SEND Cost  /  £ million" :limits [:c 0 max-max]}]
