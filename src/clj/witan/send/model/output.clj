...
;; TODO: Output this as a vec of maps as nippy
(defn format-transition-count
  "Each transtion count comes through as the *-2 side for things like cy
  and ay as that is what is being generated by the model"
  [idx [[cy-2 ay-2 need-setting-1 need-setting-2] transition-count]]
  (let [[need-1 setting-1]  (states/split-need-setting need-setting-1)
        [need-2 setting-2]  (states/split-need-setting need-setting-2)]
    [idx (dec cy-2)
     (name setting-1) (name need-1) (dec ay-2)
     (name setting-2) (name need-2) ay-2
     transition-count]))
